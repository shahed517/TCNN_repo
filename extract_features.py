...
if __name__=="__main__":
    ...
    n_mel_channels = 80  # Default for HiFi-GAN
    sampling_rate = args.audio_sr  # Default 22000 for HiFi-GAN
    hop_length = 220  # Default for HiFi-GAN to make a 0.01 sec hop
    win_length = 1100  # Default for HiFi-GAN to make a 0.05 sec long window
    n_fft = win_length  # Default for HiFi-GAN
    ...
    for p_id, participant in enumerate(participants['participant_id']): 
        ...
        print(f'hga_lfc shape : {eeg_hga_lfc.shape}')
        ...
        rms = librosa.feature.rms(y=audio, frame_length=win_length, hop_length=hop_length, center=True)[0]
        ...
        # print(eeg_segments.shape, mel_segments.shape, va_labels.shape)

        MAX_WAV_VALUE = np.max(np.abs(audio))
        scaled = audio/MAX_WAV_VALUE # normalize for hifiGAN
        scaled = torch.from_numpy(scaled).unsqueeze(0)
